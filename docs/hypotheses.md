...
H0  (replication) At λ=0, silent-wrong rates order supervisor ≤ pipeline < single < swarm, consistent with
    Kim et al.'s centralized-beats-decentralized result. If this fails, faulted results must be interpreted
    with caution and the discrepancy reported.
...
- H0/H1: silent_wrong rate per topology × λ, 95% Wilson CIs; H1 tested as a topology×λ interaction.
